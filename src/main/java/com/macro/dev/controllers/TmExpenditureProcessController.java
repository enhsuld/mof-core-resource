...
@RestController
@RequestMapping("/api/tm_expenditure_process")
public class TmExpenditureProcessController {

    private TmExpenditureProcessService service;

    @Autowired
    public TmExpenditureProcessController(TmExpenditureProcessService service) {
    ...
    @GetMapping("/{id}")
    public Optional<TmExpenditureProcess> getById(@PathVariable String id) {
        return service
                .getById(id);
    }
    ...
    public ResponseEntity<?> post(@RequestParam(value = "data", required = false) String jsonString) throws JSONException {
        ...
        TmExpenditureProcess objectClass = new TmExpenditureProcess();
    ...
    public ResponseEntity<?> put(@RequestParam(value = "data", required = false) String jsonString) throws JSONException {
        JSONObject obj = new JSONObject(jsonString);
        Gson gson = new Gson();
        TmExpenditureProcess objectClass = gson.fromJson(jsonString,TmExpenditureProcess.class);
    ...
    private void assertExist(String id) {
        service
                ...
                .orElseThrow(() -> new ResourceNotFoundException()
                        .setResourceName(ResourceNameConstant.TmExpenditureProcess)
                        .setId(id));
    }
