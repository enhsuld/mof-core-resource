...
import com.macro.dev.services.TmRevenueBaseImformationService;
...
@RestController
@RequestMapping("/api/tm_revenue_base_imformation")
public class TmRevenueBaseImformationController {

    private TmRevenueBaseImformationService service;

    @Autowired
    public TmRevenueBaseImformationController(TmRevenueBaseImformationService service) {
    ...
    @GetMapping("/{id}")
    public Optional<TmRevenueBaseImformation> getById(@PathVariable String id) {
        return service
                .getById(id);
    }
    ...
    public ResponseEntity<?> post(@RequestParam(value = "data", required = false) String jsonString) throws JSONException {
        ...
        TmRevenueBaseImformation objectClass = new TmRevenueBaseImformation();
        objectClass.setOrgCd(obj.getString("orgCd"));
        objectClass.setComCd(obj.getString("comCd"));
        objectClass.setRbiNm(obj.getInt("rbiNm"));
        objectClass.setUnitMst(obj.getString("unitMst"));
    ...
    public ResponseEntity<?> put(@RequestParam(value = "data", required = false) String jsonString) throws JSONException {
        JSONObject obj = new JSONObject(jsonString);
        Gson gson = new Gson();
        TmRevenueBaseImformation objectClass = gson.fromJson(jsonString,TmRevenueBaseImformation.class);
    ...
    private void assertExist(String id) {
        service
                ...
                .orElseThrow(() -> new ResourceNotFoundException()
                        .setResourceName(ResourceNameConstant.TmRevenueBaseImformation)
                        .setId(id));
    }
