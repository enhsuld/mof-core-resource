...
@RestController
@RequestMapping("/api/tm_macro_indicator")
public class TmMacroIndicatorController {

    private TmMacroIndicatorService service;

    @Autowired
    public TmMacroIndicatorController(TmMacroIndicatorService service) {
    ...
    @GetMapping("/{id}")
    public Optional<TmMacroIndicator> getById(@PathVariable String id) {
        return service
                .getById(id);
    }
    ...
    public ResponseEntity<?> post(@RequestParam(value = "data", required = false) String jsonString) throws JSONException {
        ...
        TmMacroIndicator objectClass = new TmMacroIndicator();
        ...
        URI location = ServletUriComponentsBuilder
                .fromCurrentRequest()
                .path("/{id}")
                .buildAndExpand(objectClass.getMiId())
                .toUri();
    ...
    public ResponseEntity<?> put(@RequestParam(value = "data", required = false) String jsonString) throws JSONException {
        JSONObject obj = new JSONObject(jsonString);
        Gson gson = new Gson();
        TmMacroIndicator objectClass = gson.fromJson(jsonString,TmMacroIndicator.class);

        assertExist(obj.getString("id"));

        service.modify(objectClass.setMiId(obj.getString("id")));
    ...
    private void assertExist(String id) {
        service
                ...
                .orElseThrow(() -> new ResourceNotFoundException()
                        .setResourceName(ResourceNameConstant.TmMacroIndicator)
                        .setId(id));
    }
