...
@RestController
@RequestMapping("/api/tm_revenue_process")
public class TmRevenueProcessController {

    private TmRevenueProcessService service;

    @Autowired
    public TmRevenueProcessController(TmRevenueProcessService service) {
    ...
    @GetMapping("/{id}")
    public Optional<TmRevenueProcess> getById(@PathVariable String id) {
        return service
                .getById(id);
    }
    ...
    public ResponseEntity<?> post(@RequestParam(value = "data", required = false) String jsonString) throws JSONException {
        ...
        TmRevenueProcess objectClass = new TmRevenueProcess();
    ...
    public ResponseEntity<?> put(@RequestParam(value = "data", required = false) String jsonString) throws JSONException {
        JSONObject obj = new JSONObject(jsonString);
        Gson gson = new Gson();
        TmRevenueProcess objectClass = gson.fromJson(jsonString,TmRevenueProcess.class);
    ...
    private void assertExist(String id) {
        service
                ...
                .orElseThrow(() -> new ResourceNotFoundException()
                        .setResourceName(ResourceNameConstant.TmRevenueProcess)
                        .setId(id));
    }
