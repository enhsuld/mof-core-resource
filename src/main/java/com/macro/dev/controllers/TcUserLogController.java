...
public class TcUserLogController {
    ...
    @Autowired
    public TcUserLogController(TcUserLogService tcFaqService) {
    ...
    @GetMapping("/{id}")
    public Optional<TcUserLog> getTcFaqById(@PathVariable String id) {
        return tcFaqService
                .getById(id);
    }
    ...
    public ResponseEntity<?> postTcFaq(@RequestParam(value = "data", required = false) String jsonString) throws JSONException {
        ...
        TcUserLog tcFaq = new TcUserLog();
        tcFaq.setUser_id(obj.getString("user_id"));
        tcFaq.setLogin_dtm(obj.getString("login_dtm"));
        tcFaq.setLogout_dtm(obj.getString("logout_dtm"));
        tcFaq.setUser_ip(obj.getString("user_ip"));
    ...
    public ResponseEntity<?> putTcFaq(@RequestParam(value = "data", required = false) String jsonString) throws JSONException {
        JSONObject obj = new JSONObject(jsonString);
        Gson gson = new Gson();
        TcUserLog tcFaq = gson.fromJson(jsonString,TcUserLog.class);
