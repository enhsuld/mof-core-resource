...
@RestController
@RequestMapping("/api/tm_code_combination")
public class TmCodeCombinationController {

    private TmCodeCombinationService service;

    @Autowired
    public TmCodeCombinationController(TmCodeCombinationService service) {
    ...
    @GetMapping("/{id}")
    public Optional<TmCodeCombination> getById(@PathVariable String id) {
        return service
                .getById(id);
    }
    ...
    public ResponseEntity<?> post(@RequestParam(value = "data", required = false) String jsonString) throws JSONException {
        ...
        TmCodeCombination objectClass = new TmCodeCombination();
        objectClass.setComCd(obj.getString("comCd"));
        objectClass.setBeCd(obj.getString("beCd"));
        ...
        URI location = ServletUriComponentsBuilder
                .fromCurrentRequest()
                .path("/{id}")
                .buildAndExpand(objectClass.getCdId())
                .toUri();
    ...
    public ResponseEntity<?> put(@RequestParam(value = "data", required = false) String jsonString) throws JSONException {
        JSONObject obj = new JSONObject(jsonString);
        Gson gson = new Gson();
        TmCodeCombination objectClass = gson.fromJson(jsonString,TmCodeCombination.class);

        assertExist(obj.getString("id"));

        service.modify(objectClass.setCdId(obj.getString("id")));
    ...
    private void assertExist(String id) {
        service
                ...
                .orElseThrow(() -> new ResourceNotFoundException()
                        .setResourceName(ResourceNameConstant.TmCodeCombination)
                        .setId(id));
    }
