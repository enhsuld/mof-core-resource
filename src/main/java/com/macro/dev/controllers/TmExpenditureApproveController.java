...
@RestController
@RequestMapping("/api/tm_expenditure_approve")
public class TmExpenditureApproveController {

    private TmExpenditureApproveService service;

    @Autowired
    public TmExpenditureApproveController(TmExpenditureApproveService service) {
    ...
    @GetMapping("/{id}")
    public Optional<TmExpenditureApprove> getById(@PathVariable String id) {
        return service
                .getById(id);
    }
    ...
    public ResponseEntity<?> post(@RequestParam(value = "data", required = false) String jsonString) throws JSONException {
        ...
        TmExpenditureApprove objectClass = new TmExpenditureApprove();
    ...
    public ResponseEntity<?> put(@RequestParam(value = "data", required = false) String jsonString) throws JSONException {
        JSONObject obj = new JSONObject(jsonString);
        Gson gson = new Gson();
        TmExpenditureApprove objectClass = gson.fromJson(jsonString,TmExpenditureApprove.class);
    ...
    private void assertExist(String id) {
        service
                ...
                .orElseThrow(() -> new ResourceNotFoundException()
                        .setResourceName(ResourceNameConstant.TmExpenditureApprove)
                        .setId(id));
    }
